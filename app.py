...
st.markdown("""
    <style>
        ...
        /*  Verrouille totalement la taille de la sidebar */
...
# 🎯 Titre + Sous-titre centré avec effet professionnel
  st.markdown("""
    <div style="text-align: center; margin-top: -100px; margin-bottom: 30px;">
        <h1 style="font-size: 55px; color: #003366; font-weight: 800;">
            🚚 <span style="color: ##1E3C72;">SupplyPilot</span>
        </h1>
        <h3 style="font-size: 35px; color: #1E3C72;">
            Application de Prédiction & <b>Suivi Logistique</b>
        </h3>
        <p style="font-size: 26px; color: #444; line-height: 1.6; max-width: 800px; margin: auto;">
        ...
        <p style="font-size: 20px; color: #555; margin-top: 10px;">
            <b>Technos :</b> Python · Streamlit · Machine Learning (eXtreme Gradient Boosting) · Feature Engineering
        </p>
    </div>
""", unsafe_allow_html=True)
  ...
  with col_left:
    st.image("assets/supplychain.png", use_container_width=True)
  ...
  with col_right:
    ...
    with st.expander("🧐 À propos du modèle utilisé"):
        st.info("""
        Modèle XGBoost entraîné sur des données internes.
    ...
    with col1:
        if mode == "✍️ Saisie manuelle":
            ...
            if st.button("📈 Prédire", key="predict_manuelle"):
                 if not code_fournisseur or not article_code:
                    st.warning("⚠️ Veuillez remplir tous les champs obligatoires : Code fournisseur et Code article.")
                 else:
                    # Encodage avec gestion des valeurs inconnues
                    if code_fournisseur in encoders['code_fournisseur'].classes_:
                        code_fournisseur_enc = encoders['code_fournisseur'].transform([code_fournisseur])[0]
                    else:
                        code_fournisseur_enc = 999

                    if article_code in encoders['article_code'].classes_:
                        article_code_enc = encoders['article_code'].transform([article_code])[0]
                    else:
                        article_code_enc = 999
...
if page == "📙 Historique des Prédictions": 
    ...
    <style>
        ...
        /* 🔄 Forcer la couleur noire dans le menu déroulant */
        ...
        /* 📊 Tableau : empêcher débordement */
        .dataframe {
            max-width: 100% !important;
            overflow-x: auto !important;
        }

        /* ✅ Ajout de marges sur le contenu principal */
        section.main > div {
            padding-left: 3rem !important;
            padding-right: 3rem !important;
        }
        
            /* ✅ Correction du style des menus déroulants */
        div[data-baseweb="select"] {
            width: 100% !important;
            max-width: 100% !important;
            padding: 6px;
            margin: 0 !important;  /* 🧨 empêche le centrage */
        }
...
if page == "🔎 Exploration":
    st.markdown("""
        <div style="text-align: center; margin-top: -140px; margin-bottom: 30px;">
            <h1 style="font-size: 50px; color: #1E3C72; font-weight: 900;">
    ...
    buf_mois.seek(0)
    st.image(Image.open(buf_mois), use_container_width=False)
    ...
    buf_best.seek(0)
    st.image(Image.open(buf_best), use_container_width=False)
    ...
    buf_worst.seek(0)
    st.image(Image.open(buf_worst), use_container_width=False)
# ================================ 📈 Performances=========================
...
if page == "📈 Performances":
    ...
    st.markdown("""
        <h1 style='text-align: center; font-size: 50px; font-weight: 900; color: #1E3C72; margin-top: -140px; margin-bottom: 30px;'>
    ...
    st.markdown("""
        ...
        ">
            <p style='font-size: 24px; line-height: 1.6;'>
                Voici les performances du modèle <b>XGBoost</b> lors des phases de test :
            </p>
            <ul style='font-size: 18px; line-height: 2.0; margin-left: 25px;'>
                ...
                <li><span style="font-size: 24px;">❄️ <b>Modèle final utilisé :</b> <span style="color:#0074A2;"><b>eXtreme Gradient Boosting</b></span></span></li>
            </ul>
        </div>
    """, unsafe_allow_html=True)
    ...
    file_path = "data/y_true_preds.csv"

    if os.path.exists(file_path):
        
        df_perf = pd.read_csv(file_path, sep=";")
        ...
        st.error("❌ Fichier manquant. Vérifie que data/y_true_preds.csv est bien dans ton GitHub.")
